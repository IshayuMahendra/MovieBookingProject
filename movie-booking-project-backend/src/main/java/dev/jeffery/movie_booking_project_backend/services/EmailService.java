...
public class EmailService {
    ...
    public void sendPromotionEmail(String email, Promotion promotion) {
        try {
            String subject = "A new promotion is available!";
            String content = """
                <div style="font-family: Arial, sans-serif; max-width:600px;margin:auto;padding:20px;border-radius:8px;background:#f9f9f9;">
                  <h2 style="margin:0 0 10px 0;color:#333;">%s</h2>
                  <p style="font-size:15px;color:#444;">A new promotion is available! Take advantage of this to receive a discount on your next ticket purchase!.</p>
                  <p style="font-size:13px;color:#666;">Promo Code: %s</p>
                  <p style="font-size:13px;color:#666;">Discount: %s%%</p>
                  <p style="font-size:13px;color:#666;">Expiration Date: %s</p>
                  <p style="font-size:12px;color:#999;margin-top:20px;">This is an automated message.</p>
                </div>
            """.formatted(
                    BRAND,
            ...
            mailSender.send(msg);
            System.out.println("New promotion email sent to " + email);
        } catch (Exception e) {
            System.err.println("Failed to send new promotion email: " + e.getMessage());
        }
    }
